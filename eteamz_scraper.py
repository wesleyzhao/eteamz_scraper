...
def insert_links(links, state, gender, age, sport = "", skill = "", organization = ""):
    insert_str = ", ".join(["('%s', '%s', '%s', '%s')" % (link, state, gender, age, sport, skill, organization) for link in links])
    query_str = "INSERT INTO teams (url, state, gender, age, sport, skill, organization) VALUES %s" % (insert_str)
    mysql = get_cursor()
    cur = mysql['cursor']
...
def mark_stop(page_num, state, gender, age, sport = "", skill = "", organization = ""):
    mysql = get_cursor()
    cur = mysql['cursor']
    cur.execute("INSERT INTO stops (page_number, state, gender, age, sport, skill, organization) VALUES (%s, '%s', '%s', '%s', '%s', '%s', '%s')" % (str(page_num), state, gender, age, sport, skill, organization))
    mysql['conn'].commit()
    return cur.rowcount
                    ...
                    for skill in skills:
                        for organization in organizations:
                            ...
                            while  last_link_len >=10:
                                url = make_url(page = page_num, gender = gender, state = state, age = age, sport = sport, skill = skill, organization = organization)
                                links = get_links(url)
                                last_link_len = len(links)
                                page_num += 1
                                if links:
                                    insert_links(links, state, gender, age, sport, skill, organization)
                                    print 'inserted -- url: %s,   gender: %s,   # of links %s,  age: %s, sport: %s, skill: %s, organization %s' % (url, gender, str(last_link_len), age, sport, skill, organization)
                            mark_stop(page_num - 1, state, gender, age, sport, skill, organization)
                            print 'Stopped at page # %s, state: %s, gender: %s, age: %s, sport: %s, skill: %s, organization: %s' %(str(page_num-1), state, gender, age, sport, skill, organization)
